...
const parseMall3Response = (res) => {
  ...
  const { code, message, errorData } = res.data
...
const parseMall3Error = (err) => {}

// 响应拦截
service.interceptors.response.use(
  (res) => {
    return parseMall3Response(res)
  },
  (err) => {
    ...
    ElMessage({
      message: errMessage ? errMessage : err,
      type: 'error',
      duration: 2 * 1000
    })
    //如果是跨域
...
export function axiosReq({
  ...
  return service({
    ...
    // baseURL: baseURL ?? (url.startsWith('/api/mall3') ? '' : import.meta.env.VITE_APP_BASE_URL),
