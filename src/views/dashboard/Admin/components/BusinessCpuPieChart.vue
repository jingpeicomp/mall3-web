...
const setOptions = (statsData) => {
  ...
  for (const systemHostStats of statsData) {
    ...
    if (businessHostStats[businessName] === undefined) {
      businessHostStats[businessName] = systemHostStats.cpuCoreNumber
    } else {
      businessHostStats[businessName] += systemHostStats.cpuCoreNumber
    }
  }
  let businessNameArray = Array.from(businessNames)
  let cpuCoreNumberData = Object.keys(businessHostStats).map(businessName => {
  ...
  state.chart.setOption({
    ...
    series: [
      {
        name: 'Business Cpu Core Number',
        type: 'pie',
        radius: [40, 140],
        center: ['50%', '42%'],
        data: cpuCoreNumberData,
